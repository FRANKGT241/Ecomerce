...
export default function ProductListClient({ initialProducts }: ProductListClientProps) {
  ...
  useEffect(() => {
    const filtered = products.filter((product) => {
      ...
      const priceMatch = product.price !== null && parseFloat(product.price as unknown as string) >= priceRange[0] && parseFloat(product.price as unknown as string) <= priceRange[1]
