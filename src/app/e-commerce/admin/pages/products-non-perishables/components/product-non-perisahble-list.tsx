  ...
  const filteredAndSortedProducts = useMemo(() => {
    const filteredProducts = products.filter((product) => {
      const matchesSearchTerm =
        (product.product_non_perishable_id?.toString() || "").includes(searchTerm) || 
        product.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        product.keywords.toLowerCase().includes(searchTerm.toLowerCase()) ||
        product.meta_description.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <Table className="border-spacing-0">
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="border-b-0">Cantidad en existencia</TableHead>
            ...
            <TableHead className="hidden md:table-cell border-b-0">Precio</TableHead> 
        ...
        <TableBody>
          {filteredAndSortedProducts.length > 0 ? (
            filteredAndSortedProducts.map((product) => {
              ...
              return (
                <TableRow key={product.product_non_perishable_id} className={isActive ? '' : 'bg-muted/50'}>
                  <TableCell className="font-medium border-b-0">{product.product_non_perishable_id}</TableCell>
                  <TableCell className="border-b-0">{product.name}</TableCell>
                  <TableCell className="border-b-0">{product.stock}</TableCell>
                  ...
                  <TableCell className="hidden md:table-cell border-b-0"> {typeof product.price === "number" ? product.price.toFixed(2) : parseFloat(product.price).toFixed(2)}
                  </TableCell>
                  ...
                  <TableCell className="border-b-0">
                    <div className="flex flex-col sm:flex-row gap-2">
                      <Button variant="outline" size="sm" onClick={() => onEdit(product.product_non_perishable_id)}>
                        Editar
                      </Button>
                      <Button
                        className={isActive ? "bg-red-500 hover:bg-red-600 text-white" : "bg-green-500 hover:bg-green-600 text-white"}
                        onClick={() => onDelete(product.product_non_perishable_id)}
                      >
            ...
            <TableRow>
              <TableCell colSpan={8} className="text-center border-b-0">
